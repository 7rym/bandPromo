...
const BANDPROMO_BRAND_REGISTRY_VERSION = 1;
...
{
    $id = bandpromo_brand_normalize_id($id);
...
function bandpromo_brand_legacy_theme_id(string $id): string
{
    ...
    return $id === BANDPROMO_BRAND_DEFAULT_ID ? BANDPROMO_BRAND_DEFAULT_ID : $id;
}
...
function bandpromo_brand_legacy_storage_root(string $root): string
{
...
{
    return bandpromo_brand_storage_root($root) . DIRECTORY_SEPARATOR . 'registry.json';
}

function bandpromo_brand_document_path(string $root, string $themeId): string
{
    $themeId = bandpromo_brand_canonical_id($themeId);

    return bandpromo_brand_storage_root($root) . DIRECTORY_SEPARATOR . $themeId . '.json';
}

function bandpromo_brand_legacy_document_path(string $root, string $themeId): string
{
    return bandpromo_brand_legacy_storage_root($root) . DIRECTORY_SEPARATOR . bandpromo_brand_normalize_id($themeId) . '.json';
}

function bandpromo_brand_template_path(string $root, string $themeId): string
{
    return $root . DIRECTORY_SEPARATOR . 'biblioteca' . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR
        . bandpromo_brand_normalize_id($themeId) . '.theme.template.json';
}

function bandpromo_brand_registry_ensure_dir(string $root): void
{
    $dir = bandpromo_brand_storage_root($root);
    if (!is_dir($dir) && !mkdir($dir, 0750, true) && !is_dir($dir)) {
        throw new RuntimeException('Could not create data/brands directory.');
    }
}

function bandpromo_brand_normalize_id(string $themeId): string
{
...
{
    $value = bandpromo_brand_normalize_id($value);
...
function bandpromo_brand_default_color_tokens(): array
{
    return [
        'primary' => '#00d2ff',
        'secondary' => '#3a7bd5',
...
function bandpromo_brand_css_variable_map(): array
{
    return [
 ...
 */
function bandpromo_brand_effects_css_variables(array $document): array
{
    $dim = (int) bandpromo_brand_normalize_int_token(
        bandpromo_brand_token_value($document, 'effects.backdrop_dim'),
        0,
        100,
        72
    );
...
function bandpromo_brand_derived_alpha_css_variables(): array
{
    $variables = [];
...
function bandpromo_brand_default_document(): array
{
    return [
        'version' => BANDPROMO_BRAND_REGISTRY_VERSION,
        ...
        'tone_notes' => 'Neutral platform defaults; duplicate and customise as campaign identity.',
        'tokens' => [
        ...
        // Shell slot → registry asset id (resolve via bandpromo_brand_resolve_shell_slot_url).
        'asset_ids' => [
            'logo' => '',
            'poster' => '',
            'background_image' => '',
...
function bandpromo_brand_normalize_playlist_selector_mode(mixed $value): string
{
 ...
 */
function bandpromo_brand_legacy_playlist_selector_fallback(): string
{
    try {
        if (function_exists('get_config')) {
            $raw = get_config('player.playlist_selector', '');
            if (is_string($raw) && trim($raw) !== '') {
                return bandpromo_brand_normalize_playlist_selector_mode($raw);
            }
        }
...
function bandpromo_brand_normalize_player(array $input): array
{
    ...
    if ($raw === null || trim((string) $raw) === '') {
        $selector = bandpromo_brand_legacy_playlist_selector_fallback();
    } else {
        $selector = bandpromo_brand_normalize_playlist_selector_mode($raw);
    }
...
function bandpromo_brand_default_registry(): array
{
    return [
        'version' => BANDPROMO_BRAND_REGISTRY_VERSION,
        'brands' => [
...
function bandpromo_brand_normalize_hex_color(string $value, string $fallback): string
{
    $value = trim($value);
...
function bandpromo_brand_normalize_font_family(string $value, string $fallback = '', bool $allowEmpty = false): string
{
...
function bandpromo_brand_normalize_tokens(array $tokens): array
{
    ...
    $fontBase = bandpromo_brand_normalize_font_family(
        (string) ($typography['font_family_base'] ?? $defaultFontBase),
        $defaultFontBase
    );
    $fontHeading = bandpromo_brand_normalize_font_family(
...
function bandpromo_brand_normalize_assets(array $assets): array
{
    $defaults = bandpromo_brand_default_document()['assets'];
 ...
 * @return array<string, string>
 */
function bandpromo_brand_normalize_asset_ids(array $assetIds): array
{
    require_once __DIR__ . '/asset-registry.php';

    $defaults = bandpromo_brand_default_document()['asset_ids'];
    $normalized = [];
 ...
 * @return list<string>
 */
function bandpromo_brand_normalize_library_asset_ids(array $assetIds): array
{
    require_once __DIR__ . '/asset-registry.php';
 ...
 * @return list<string>
 */
function bandpromo_brand_merge_library_slot_assets(array $slotAssetIds, array $libraryAssetIds): array
{
    ...
    return bandpromo_brand_normalize_library_asset_ids($libraryAssetIds);
}
 ...
 */
function bandpromo_brand_add_assets_to_library(string $root, string $brandId, array $assetIds): array
{
    ...
    $library = is_array($document['library_asset_ids'] ?? null)
        ...
        : [];
    $before = bandpromo_brand_normalize_library_asset_ids($library);
    $merged = bandpromo_brand_normalize_library_asset_ids(array_merge($before, $assetIds));
    if ($merged === $before) {
        return $before;
    }
 ...
 */
function bandpromo_brand_resolve_shell_slot_url(string $root, array $document, string $slotKey): string
{
    require_once __DIR__ . '/asset-registry.php';
    ...
    if ($assetId === '' && $pathFallback !== '') {
        $assetId = bandpromo_brand_lookup_asset_id_for_path($root, $pathFallback);
    }
 ...
 */
function bandpromo_brand_shell_slot_visual_role(string $slotKey): string
{
    $map = [
        'logo' => 'brand-logo',
 ...
 */
function bandpromo_brand_clone_asset_file(string $root, string $brandId, string $assetKey, string $sourcePath, string $sourceAssetId = ''): array
{
    require_once __DIR__ . '/asset-registry.php';
    ...
    if (!is_array($sourceAsset) && $sourcePath !== '') {
        $lookedUp = bandpromo_brand_lookup_asset_id_for_path($root, $sourcePath);
        if ($lookedUp !== '') {
    ...
    if (($absolute === null || $absolute === '' || !is_file((string) $absolute)) && $sourcePath !== '') {
        $absolute = bandpromo_brand_resolve_media_absolute_path($root, $sourcePath);
        if ($absolute === null) {
            $seedFallbacks = bandpromo_brand_shell_seed_fallback_paths()[$assetKey] ?? [];
            $healed = bandpromo_brand_heal_media_path($root, $sourcePath, $seedFallbacks);
            if ($healed !== '') {
                $absolute = bandpromo_brand_resolve_media_absolute_path($root, $healed);
            }
        }
    }
    ...
    $role = bandpromo_brand_shell_slot_visual_role($assetKey);
 ...
 */
function bandpromo_brand_clone_assets_for_brand(string $root, array $assets, string $brandId, array $sourceAssetIds = []): array
{
    $brandId = bandpromo_brand_canonical_id($brandId);
    if ($brandId === '') {
    ...
    foreach ($defaults as $key => $_defaultValue) {
        $result = bandpromo_brand_clone_asset_file(
            $root,
            $brandId,
            $key,
    ...
    return [
        'assets' => bandpromo_brand_normalize_assets($clonedAssets),
        'asset_ids' => bandpromo_brand_normalize_asset_ids($clonedIds),
    ];
}
 ...
 */
function bandpromo_brand_lookup_asset_id_for_path(string $root, string $webPath): string
{
    require_once __DIR__ . '/asset-registry.php';
 ...
 */
function bandpromo_brand_materialize_asset_urls(string $root, array $document): array
{
 ...
 */
function bandpromo_brand_resolve_media_absolute_path(string $root, string $webPath): ?string
{
    $webPath = trim(str_replace('\\', '/', $webPath));
    if ($webPath === '' || preg_match('#^https?://#i', $webPath) === 1) {
        return null;
...
function bandpromo_brand_normalize_media_web_path(string $path): string
{
...
function bandpromo_brand_first_existing_media_path(string $root, array $candidates): string
{
    foreach ($candidates as $candidate) {
        $normalized = bandpromo_brand_normalize_media_web_path((string) $candidate);
        if ($normalized !== '' && bandpromo_brand_resolve_media_absolute_path($root, $normalized) !== null) {
            return $normalized;
        }
    }
 ...
 */
function bandpromo_brand_heal_media_path(string $root, string $path, array $fallbacks): string
{
    $normalized = bandpromo_brand_normalize_media_web_path($path);
    if ($normalized !== '' && bandpromo_brand_resolve_media_absolute_path($root, $normalized) !== null) {
        return $normalized;
    }

    return bandpromo_brand_first_existing_media_path($root, $fallbacks);
}
 ...
 */
function bandpromo_brand_heal_install_shell_media(string $root): array
{
    $notes = [];
    $fallbacks = bandpromo_brand_shell_seed_fallback_paths();
    $activeId = bandpromo_brand_canonical_id(bandpromo_brand_active_id($root));
    ...
    foreach (bandpromo_brand_registry_entries($root) as $entry) {
        ...
        foreach ($fallbacks as $slot => $candidates) {
            $before = bandpromo_brand_normalize_media_web_path((string) ($assets[$slot] ?? ''));
            ...
            $pathForId = bandpromo_brand_normalize_media_web_path((string) ($assets[$slot] ?? $after));
            $currentId = trim((string) ($assetIds[$slot] ?? ''));
            if ($currentId === '' && $pathForId !== '') {
                $found = bandpromo_brand_lookup_asset_id_for_path($root, $pathForId);
                if ($found !== '') {
                    $assetIds[$slot] = $found;
                    $changed = true;
    ...
    foreach ($configMap as $dotted => $candidates) {
        $before = bandpromo_brand_normalize_media_web_path((string) bandpromo_config_get_path($config, $dotted, ''));
        $after = bandpromo_brand_heal_media_path($root, $before, $candidates);
        if ($after !== '' && $after !== $before) {
...
function bandpromo_brand_normalize_document(array $input, ?string $expectedId = null): array
{
    ...
    $libraryAssetIds = bandpromo_brand_merge_library_slot_assets(
        $assetIds,
        bandpromo_brand_normalize_library_asset_ids(
    ...
    return [
        ...
        'tokens' => bandpromo_brand_normalize_tokens(is_array($input['tokens'] ?? null) ? $input['tokens'] : []),
        'assets' => bandpromo_brand_normalize_assets(is_array($input['assets'] ?? null) ? $input['assets'] : []),
        'asset_ids' => $assetIds,
        'library_asset_ids' => $libraryAssetIds,
        'player' => bandpromo_brand_normalize_player($input),
    ];
}

function bandpromo_brand_write_registry(string $root, array $registry): void
{
    bandpromo_brand_registry_ensure_dir($root);
    if (!bandpromo_json_write_file(bandpromo_brand_registry_path($root), $registry)) {
        throw new RuntimeException('Could not write data/brands/registry.json');
    }
}

function bandpromo_brand_registry_list_key(array $registry): string
{
...
function bandpromo_brand_load_registry(string $root): array
{
    bandpromo_brand_registry_ensure_dir($root);
    $decoded = bandpromo_json_read_array_file(bandpromo_brand_registry_path($root));
    if ($decoded === null) {
        $default = bandpromo_brand_default_registry();
        bandpromo_brand_write_registry($root, $default);

        return $default;
    }

    $listKey = bandpromo_brand_registry_list_key($decoded);
    ...
    if ($listKey === 'themes') {
        bandpromo_brand_write_registry($root, $decoded);
    }
 ...
 */
function bandpromo_brand_write_document(string $root, array $document, array $options = []): void
{
    $document = bandpromo_brand_normalize_document($document, (string) ($document['id'] ?? ''));
    ...
    bandpromo_brand_registry_ensure_dir($root);
    if (!bandpromo_json_write_file(bandpromo_brand_document_path($root, $document['id']), $document)) {
...
{
    $path = bandpromo_brand_document_path($root, BANDPROMO_BRAND_DEFAULT_ID);
    if (!is_file($path)) {
        return;
    }
    ...
    try {
        $document = bandpromo_brand_load_document($root, BANDPROMO_BRAND_DEFAULT_ID);
    } catch (Throwable $throwable) {
        return;
    }
 ...
 */
function bandpromo_brand_api_document(array $document): array
{
...
function bandpromo_brand_load_document(string $root, string $themeId): array
{
    $themeId = bandpromo_brand_canonical_id($themeId);
    if ($themeId === '') {
        throw new InvalidArgumentException('Invalid brand id.');
    ...
    $path = bandpromo_brand_document_path($root, $themeId);
    if (!is_file($path)) {
    ...
    $document = bandpromo_brand_normalize_document($decoded, $themeId);
    $materialized = bandpromo_brand_materialize_asset_urls($root, $document);
...
function bandpromo_brand_registry_entries(string $root): array
{
    ...
    foreach ($entries as $index => $entry) {
        ...
        try {
            $document = bandpromo_brand_load_document($root, $canonical);
...
function bandpromo_brand_registry_entry(string $root, string $themeId): ?array
{
    ...
    foreach (bandpromo_brand_load_registry($root)['brands'] ?? [] as $entry) {
...
function bandpromo_brand_assets_from_config(array $config): array
{
...
function bandpromo_brand_sync_assets_to_config(string $root, array $document): void
{
    $configPath = $root . '/web-config.json';
    $config = bandpromo_load_runtime_config_raw($configPath);
    if ($config === []) {
...
function bandpromo_brand_active_canonical_id(string $root): string
{
    return bandpromo_brand_canonical_id(bandpromo_brand_active_id($root));
}

function bandpromo_brand_set_active_id(string $root, string $themeId): void
{
    $themeId = bandpromo_brand_canonical_id($themeId);
    if ($themeId === '' || !is_file(bandpromo_brand_document_path($root, $themeId))) {
...
function bandpromo_brand_load_active_document(string $root): array
{
    ...
    return bandpromo_brand_load_document($root, bandpromo_brand_active_id($root));
}

function bandpromo_brand_token_value(array $document, string $path): string
{
...
{
    $brandsRoot = bandpromo_brand_storage_root($root);
    $legacyRoot = bandpromo_brand_legacy_storage_root($root);
    ...
    if (is_file($legacyRegistryPath)) {
        ...
        if ($brands === []) {
            $brands = bandpromo_brand_default_registry()['brands'];
        }
        bandpromo_brand_write_registry($root, [
            'version' => BANDPROMO_BRAND_REGISTRY_VERSION,
            'brands' => $brands,
        ]);
    }
    ...
    foreach (glob($legacyRoot . DIRECTORY_SEPARATOR . '*.json') ?: [] as $legacyDocPath) {
        ...
        $target = bandpromo_brand_document_path($root, $canonical);
        if (is_file($target)) {
            continue;
        }
        ...
        bandpromo_json_write_file($target, bandpromo_brand_normalize_document($decoded, $canonical));
    }
}

function bandpromo_brand_migrate_from_config(string $root): void
{
    $defaultPath = bandpromo_brand_document_path($root, BANDPROMO_BRAND_DEFAULT_ID);
    if (is_file($defaultPath)) {
        return;
    }

    $templatePath = bandpromo_brand_template_path($root, BANDPROMO_BRAND_DEFAULT_ID);
    ...
    if (is_file($templatePath)) {
        ...
        $document = is_array($decoded)
            ? bandpromo_brand_normalize_document($decoded, BANDPROMO_BRAND_DEFAULT_ID)
            : bandpromo_brand_default_document();
    } else {
        $document = bandpromo_brand_default_document();
    }

    $config = bandpromo_load_runtime_config_raw($root . '/web-config.json');
    if ($config !== []) {
        $document['assets'] = bandpromo_brand_normalize_assets(
            array_merge($document['assets'], bandpromo_brand_assets_from_config($config))
        );
    }
    ...
    if ($config !== []) {
        ...
        if (trim((string) bandpromo_config_get_path($config, 'install.pointers.active_theme_id', '')) === '') {
            bandpromo_config_set_path($config, 'install.pointers.active_theme_id', BANDPROMO_BRAND_DEFAULT_ID);
            $changed = true;
        }
        if ($changed) {
            bandpromo_json_write_file($configPath, $config);
 ...
 */
function bandpromo_brand_migrate_library_asset_ids(string $root): void
{
    require_once __DIR__ . '/asset-registry.php';
    ...
    foreach (bandpromo_brand_registry_entries($root) as $entry) {
        ...
        $document = bandpromo_brand_normalize_document($raw, $brandId);
        $library = is_array($document['library_asset_ids'] ?? null)
            ? $document['library_asset_ids']
            : [];
...
function bandpromo_brand_ensure_seeded(string $root): void
{
    ...
    bandpromo_brand_migrate_from_themes($root);
    bandpromo_brand_registry_ensure_dir($root);
    if (!is_file(bandpromo_brand_registry_path($root))) {
        bandpromo_brand_write_registry($root, bandpromo_brand_default_registry());
    }

    if (!is_file(bandpromo_brand_document_path($root, BANDPROMO_BRAND_DEFAULT_ID))) {
        bandpromo_brand_migrate_from_config($root);
    }
...
function bandpromo_brand_slug_from_title(string $title): string
{
...
function bandpromo_brand_propose_duplicate_title(string $sourceTitle): string
{
...
function bandpromo_brand_allocate_duplicate_id(string $root, string $title = ''): string
{
    ...
    for ($attempt = 0; $attempt < 100; $attempt++) {
        ...
        if ($id === '' || $id === BANDPROMO_BRAND_DEFAULT_ID || $id === BANDPROMO_BRAND_DEFAULT_ID) {
            continue;
        }
        if (!isset($existing[$id]) && !is_file(bandpromo_brand_document_path($root, $id))) {
            return $id;
        }
    }
...
function bandpromo_brand_duplicate(string $root, string $sourceId, string $newId, string $title = ''): array
{
    ...
    $source = bandpromo_brand_load_document($root, $sourceId);
    ...
    if (is_file(bandpromo_brand_document_path($root, $newId))) {
        throw new InvalidArgumentException('Brand id already exists.');
    }

    $duplicate = bandpromo_brand_normalize_document([
        'id' => $newId,
    ...
    $duplicate['library_asset_ids'] = bandpromo_brand_merge_library_slot_assets(
        $duplicate['asset_ids'],
        is_array($duplicate['library_asset_ids'] ?? null) ? $duplicate['library_asset_ids'] : []
    );

    bandpromo_json_write_file(bandpromo_brand_document_path($root, $newId), $duplicate);
...
function bandpromo_brand_update_title(string $root, string $themeId, string $title): array
{
    ...
    return bandpromo_brand_registry_entry($root, $themeId) ?? [];
}

function bandpromo_brand_delete(string $root, string $themeId): void
{
    $themeId = bandpromo_brand_canonical_id($themeId);
