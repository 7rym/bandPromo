 ...
 *   {
 *     "site": { "name", "short_name", "description", "url", "author", "email", "email_auto" }
 *   }
...
if (isset($body['site']) && is_array($body['site'])) {
    ...
    $simpleFields = ['name', 'short_name', 'description', 'url', 'author'];
    ...
    if (array_key_exists('email', $body['site'])) {
        $config['site']['email'] = trim((string) $body['site']['email']);
    }

    if (array_key_exists('email_auto', $body['site'])) {
        $config['site']['email_auto'] = bandpromo_site_contact_bool($body['site']['email_auto']);
    }

    $contactError = bandpromo_site_prepare_contact_fields($config['site']);
    if ($contactError !== null) {
        http_response_code(400);
        echo json_encode(['ok' => false, 'error' => $contactError]);
        exit;
    }
}
