(function () {
    ...
    if (playerLayoutCard) {
        ...
        function renderLockedRow(item, position) {
            return `<li class="playlist-editor-row player-layout-row-locked" data-tab-key="${escapeHtml(item.key)}">
        ...
        function getAvailableRows() {
            ...
            return Array.from(availableList.querySelectorAll('.playlist-editor-row[draggable="true"]'));
        }
        ...
        function renderActiveRow(item, position) {
            const selectedClass = selectedActive.has(item.key) ? ' playlist-editor-row-selected' : '';
            return `<li class="playlist-editor-row player-layout-row-active${selectedClass}" draggable="true" data-tab-key="${escapeHtml(item.key)}" data-kind="${escapeHtml(item.kind)}" data-id="${escapeHtml(item.id)}" aria-selected="${selectedActive.has(item.key) ? 'true' : 'false'}">
        ...
        function renderAvailableRow(item) {
            const selectedClass = selectedAvailable.has(item.key) ? ' playlist-editor-row-selected' : '';
            return `<li class="playlist-editor-row${selectedClass}" draggable="true" data-tab-key="${escapeHtml(item.key)}" data-kind="${escapeHtml(item.kind)}" data-id="${escapeHtml(item.id)}" aria-selected="${selectedAvailable.has(item.key) ? 'true' : 'false'}">
                <span class="playlist-drag-handle" title="Drag into player tabs">⠿</span>
                <span class="playlist-track-info">
                    <strong>${escapeHtml(itemTitle(item))}</strong>
                    <span class="playlist-track-meta">${escapeHtml(itemMeta(item))}</span>
                </span>
