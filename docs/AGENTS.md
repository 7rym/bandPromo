...
**Operator page headings:** under the main / Content nav bar, prefer a **breadcrumb line** (`{Section} > Pool` / `{Section} > Editor`) with optional `trailing` chips after the crumb and `actions` (← Back + Save\|Saved, plus ★ Set as default/base when present) on the right while editing. Use `bandpromo_admin_render_content_breadcrumb()` + `bandpromoContentEditorBreadcrumb.attach()`; root crumb returns to Pool on the same leave path as ← Back. See [ADMIN-UI.md](ADMIN-UI.md).
