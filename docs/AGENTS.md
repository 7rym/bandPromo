...
- Operators can verify host facts and repair managed stubs from **System → Environment** (legacy Security; `environment-report.php`, `security-sanity-check.php` / `security-sanity-repair.php`). Repair overwrites drifted managed stubs only; it never rewrites `web-config.json`.
...
- Default to the fast startup path first: run `scripts/session-start.ps1` to sync the repo, start the dev server, and gather the standard environment/worktree/backlog summary (**do not bump the session number** unless the operator explicitly asked to start a session — then pass `-BumpSession`). If [SESSION-HANDOFF.md](SESSION-HANDOFF.md) exists, that file is the exact resume point — read it before older TODO items. Use `scripts/session-end.ps1` for publishable checkpoints.
...
- **No speculative fallbacks / dual-read shims:** do not automatically add “compat”, migrate-and-keep-old, silent URL remaps, or dual-read paths “just in case.” Prefer **deleting** redundant stale logic. Ask the operator first unless the need is obviously required for live fleet safety. Closed-beta installs are semi-controlled; leftover shims accumulate faster than they help. See [PLATFORM-MODEL.md](PLATFORM-MODEL.md) → Player Campaign navigator / URLs.
...
Do not rename existing code identifiers, CSS properties, JSON keys, file names, or APIs to match UK spelling (`catalog_id`, `demo-catalog-state.php`, `buildCatalog.py`, CSS `color`, HTML/JS ids). New identifiers may keep the established `catalog` / `color` technical names so those APIs stay consistent. Operator-facing labels next to those identifiers still use UK English (Content → **Catalogue**, System → Status **Site health**).

**Operator-friendly copy:** default admin copy assumes a non-tech-savvy operator — explain plainly what an action will do. When the account has the **developer** role, optional extra detail is fine if it helps; keep primary confirms and hub blurbs readable without it. See [OPERATOR-MESSAGING.md](OPERATOR-MESSAGING.md) → Voice.
...
**Operator page headings:** under the main / Content nav bar, prefer a **breadcrumb line** (`📄 Content > {Section} > Pool` / `… > Editor`) with optional `trailing` chips or meta after the crumb and `actions` (← Back + Save\|Saved, plus ★ Set as default/base when present) on the right while editing. Prefer the **entity name** as an editable `.content-editor-name-input` at the crumb tail (Pages) or in the left edit header; not a static heading and not only a Title field in Base info. Use `bandpromo_admin_render_content_breadcrumb()` + `bandpromoContentEditorBreadcrumb.attach()`; Content and section crumbs return to Pool on the same leave path as ← Back. Split column headers (Available / Live preview) share the slate panel wash. See [ADMIN-UI.md](ADMIN-UI.md).

**Sticky chrome:** toolbars that stick over scrolling content use **opaque** `--admin-sticky-chrome-bg` (`#0f172a`) — never translucent slate wash (rows show through). Do not nest sticky under sticky on the same axis; page-builder add-block sits under the breadcrumb (`--page-builder-sticky-top`), and rich-text bars stay in-flow with their block. See [ADMIN-UI.md](ADMIN-UI.md) → Sticky toolbars and `.cursor/rules/sticky-toolbars.mdc`.

**Same-factory editors + Close vs Abort:** mutating editors share one chrome pattern (save/status top-right of the working head; footer for leave/secondary/destructive). Prefer **save on close** (Done / ✕ / backdrop / ← Back flush), and always offer **Abort** (modal) or **Discard** (unsaved leave modal) so operators can leave without writing. Do not ship a lone Close that always saves (or never saves) with no discard path. Coral accent is easy to confuse with danger red — prefer amber Save / green Saved / grey secondary / red only for irreversible. `#adminConfirmModal` must stack **above** Files editors so discard/delete confirms are not buried. Full contract: [ADMIN-UI.md](ADMIN-UI.md) → Same-factory editor chrome / Close vs Abort.

**Compactness:** admin layout padding is **0 / 4 / 8px max** (prefer none when a border already separates). Do not add 12–24px card insets on new or retouched chrome. Gaps follow the same cap. See [ADMIN-UI.md](ADMIN-UI.md) → Compactness.

**Operator control chrome:** labels end with a trailing **colon** (`Label:`), and the label sits on the **same line** as its control whenever the panel width allows (Branding `brand-effect-field--inline` / `brand-chrome-inline-row` / `brand-token-field--inline`). Prefer segmented **toggle buttons** (e.g. Branding `brand-player-setting-toggle`) over checkboxes. Prefer toggles over `<select>` dropdowns when there are **5 or fewer** alternatives. Use a dropdown (or free-text) when there are more than 5 options, the list is open-ended, or values are custom (e.g. font stacks). For continuous numeric scales (dim %, blur px), prefer an **inline slider**: `Label:` + live value + unit on the same line as the range; if the scale collapses to ≤5 named presets, prefer toggles instead. **Date fields:** shared ISO picker (`.date-input-shell`) — calendar on the left, padding clears the icon, compact width for `YYYY-MM-DD` (Captured is the exemplar); see [ADMIN-UI.md](ADMIN-UI.md) → Date fields. **Form grids:** prefer flat peer rows (not multi-field stacks in one grid cell). **Modal long textareas:** autosize to content; the modal body is the only scroller. **Markdown hints** trail the field label or toggler on the same line.
...
- **Session start (operator-requested only)** bumps the session number with `python scripts/bump_session.py` via `session-start.ps1 -BumpSession` (build number unchanged). Agent resume / new Cursor chats must **not** bump the session — default `session-start.ps1` leaves `VERSION` alone.
...
- **Adding speculative dual-read / legacy URL / “just in case” fallbacks** without asking — prefer a hard cut and delete stale paths (see Key Conventions).
