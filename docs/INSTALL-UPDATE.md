...
Bundled demo content (locked release `bandpromo-demo`) is installed from **`bandPromo-demo.pcf`** during setup (first build with `ensure_demo`; legacy `bandPromo-demo.prp` still accepted). The entire `/media` tree is git-ignored; demo media never resides in the repository and is not treated as operator uploads in git.
...
- import **`bandPromo-demo.pcf`** on the first setup build (`ensure_demo`) from the durable GitHub **`demo-content`** release (fail clearly if missing — no template seed fallback; fall back to published legacy `.prp`)
...
The seeded demo content is intentional. It is part of first-run verification and helps confirm that playback, theming, and the site shell are working on the real host. Bootstrap downloads **`bandPromo.zip`** (application only). Demo audio/media travel in the separate Demo PCF — never as git-tracked binaries, because `/media` is fully ignored. Later Admin **Publish** builds do **not** re-download the Demo PCF; they use content already on the host. **Site update** may refresh the Demo PCF when a newer `demo-content` package is published (SHA marker mismatch).
...
- **Demo PCF refresh** when the durable GitHub `demo-content` package SHA differs from `data/demo-release-package.json` (locked platform demo only; skipped if unlocked on localhost)
