...
2. Ensure the server can run PHP 8+, and if you are testing the bootstrap path, make sure `ZipArchive`, outbound HTTPS download support (`curl` or `allow_url_fopen`), and a writable target folder are available.
