...
| Pattern | Behavior |
|---------|----------|
| Toolbar | Shared `.audio-pool-toolbar` density; type chips (Visual: All + Images/Video icons; Brand assets: All + Still/Living/Sound-effects icons) + catalogue/brand filter (**All campaigns** / **All brands**) + title search + Grid/List. Brand assets **Add existing** is hidden until a Brand is selected. List view adds an **S / M / L** thumbnail-size toggle (70 / 100 / 125 px; default M). Preference is stored in `localStorage` (`bandpromo_pool_thumb_size`). List rows keep title, In use/Unused, and row actions on one line. |
