...
3. Review open → findings are **ticked by default** (Apply only those selected); **Apply treatment** (green) + **Not now** + **Back up first…** sit under the panel. Multi-campaign orphan homes stay Manual: each clash row shows a visual thumb or audio preview when available, the filename, campaigns that use it, and a green **Select catalogue home:** label on the choice until a campaign is chosen — then a green **Set catalogue home** button appears (confirm names the campaign; no Files detour). Data-container orphans stay Manual: green **Select campaign:** on the choice, then **Adopt** appears; **Delete…** stays a quiet secondary control (danger only in the confirm). Row cards are not amber-nested inside the finding — hairline separators only. Ephemeral `data/` leftovers and unambiguous container registry fixes stay ticked for Apply.  
