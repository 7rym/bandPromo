...
| **The ugly** | Grey | Janitor leftovers (orphan delivery, leftover intake under `original/`, stray ZIPs, empty dirs, non-media junk). |
...
| Masters only | Files lists, previews, pickers, and downloads are **masters only**. There is no Original\|Master list toggle. Leftover intake is reclaimable junk from **System → Status → Storage** (and Site health janitor Review → Apply). |
