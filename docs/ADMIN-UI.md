...
| Pattern | Behavior |
|---------|----------|
| Toolbar | Shared `.audio-pool-toolbar` density; type chips (Visual: All + Images/Video icons) + catalogue/brand filter + title search + Grid/List. List view adds an **S / M / L** thumbnail-size toggle (70 / 100 / 125 px; default M). Preference is stored in `localStorage` (`bandpromo_pool_thumb_size`). List rows keep title, In use/Unused, and row actions on one line. |
