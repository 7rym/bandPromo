...
2026-08-19 20:32 - Build pipeline fix after Phase 1 terminology migration: restored missing `bandpromo_brand_css_variables()` / `bandpromo_brand_player_styles_for_ids()` in brand-storage (lost during theme→brand merge), fixed asset-registry repair call (`bandpromo_campaign_repair_catalog_release_ids`), and updated Python build scripts (`makePlaylists.py`, `playlistPreview.py`, `initialSiteSeed.py`) to use campaign naming with `release_id` registry fallback; `get-playlist-preview.php` accepts `campaign` query param.
