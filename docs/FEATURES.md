...
- Content → Branding editor: pool, colours/typography tokens; **Readability** (backdrop dim, panel dim, panel blur); **Shell media** slots (logo, poster, still/living backgrounds, welcome/logged-in **Sound effects**) use the shared media picker (same ✎ flow as covers) and store Visual/SFX **delivery** URLs (`card` / stream / optimal), not `/media/special/` originals; **Playlist selector** style (Dropdown / Buttons / Cover flow) is brand-owned — Base brand drives `/play`. **Panels** colour plus Panel dim emit `--panel-fill` for transport and content glass (not black-only scrim). Accent alpha CSS vars are derived from Primary/Secondary automatically. Main/heading family stacks apply at runtime and are syntax-validated; `#mediaplayer` layout stays platform-owned (Content button stylers Phase 2). Upload under Files → Brand assets / Visual / Sound effects. Live preview shows tokens plus mobile-style player chrome and shell backdrop (living delivery stream when that slot is assigned, else still), with backdrop dim as the shell overlay and panel dim/blur on preview glass panels. The Branding preview plays living video even when the OS asks to reduce motion, so operators can judge the assignment; `/play` still falls back to still for reduced-motion visitors. Brand **description** is edited under Branding → **Base info** (stored as `mood`; shown on Catalogue campaign preview). `keywords` / `tone_notes` stay in storage for future premade themes / AI helpers and remain hidden from the editor. Duplicating a brand copies its containers and shares stable library media IDs. Saving the **base** brand syncs shell paths into `web-config.json`. Old `?tab=settings&ctab=theme` redirects here.
