...
- **Block-based Pages editor** (v0.8 beta): Text, Picture, Picture + text, Video, Gallery, and List blocks; Width/Flow including Full row; gallery Grid/List/Carousel (Parallax still compact tiles); sticky Page builder chrome and live preview; delete confirmations for pages and blocks
...
- Gallery **module blocks** on pages (grid at original photo ratios with optional max-across columns, list rows, carousel snap with peek + dots and optional in-view autorotate Slow/Normal/Fast, parallax still compact tiles) — dedicated Gallery player tab already removed
