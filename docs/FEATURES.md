...
- **Notifications** operator inbox for **live** work only: media preparation, Site update, publish follow-ups, validation. The Welcome setup checklist stays on the Welcome page and is not mirrored into the bell. Hot paths stay read-only (lite by default; no catalogue repair / materialize from the inbox).
...
- Build actions under **System → Status**: calm **Site health** (inventory + one next step), **Refresh site files** (background Python; thin PHP start; consent before run; heartbeat progress; cooperative Stop). **Peek under the hood** holds the optional machine log and developer **Repair catalogue** (Preview dry-run, then confirm Apply). Apply still runs the PHP CLI pipeline under a Python supervisor with heartbeats until Apply is fully ported. Missing visual **delivery** thumbnails need Refresh, not Repair.
- **System → Environment**: host environment report (PHP/Python/ffmpeg/launch path; best-effort disk/load/CPU/RAM; **host egress** latency/throughput distinct from the login visitor speed test; active Publish/Optimize/Repair locks) and install host-protection sanity check (managed `.htaccess` / `.user.ini` vs templates) with optional preview/repair (developer-only; legacy `?stab=security` redirects here)
- **Multi-operator safety**: Publish / Optimize use `log/build.lock` / `log/optimize.lock`. Manual Repair Preview/Apply shares `log/catalog-repair.lock` with background catalogue prep (second operator gets busy). Site update refuses while any of those locks are active — it does not kill a live build.
- Admin-panel **package updater** for hosted operators (immutable release packages); after install, refreshes the locked platform Demo PCF when the published `demo-content` SHA is newer
