...
- Build actions under **System → Status**: **Refresh site files** (catalogue → audio/image optimize → **SFX delivery** → video → playlists → visual catch-up → social → PWA). **Repair catalogue** is developer-only recovery, not an operator health check. On short PHP CPU limits (e.g. 30s), Apply budgets heavy steps and may ask for another Apply pass instead of aborting. Missing visual **delivery** thumbnails need Refresh, not Repair.
