...
- Player **page tabs** (Bio, Gallery-as-page, custom pages) come from pages associated to the **playing playlist’s release** (Catalogue → Release editor → Pages; association order is tab order). Playlist and Lyrics stay fixed. Legacy site-wide `player.tab_order` / `show_in_player` remains a fallback when a playlist has no release-owned pages. Tabs ship as empty shells; the active tab hydrates on DOMContentLoaded, others on first open — so gallery stills/posters do not contend with first paint. Gallery video files still load only in the lightbox.
