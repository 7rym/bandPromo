...
- Build actions under **System → Status**: **Refresh site files** (catalogue → audio/image optimize → **SFX delivery** → video → playlists → visual catch-up → social → PWA). **Repair catalogue** is developer-only recovery, not an operator health check. **Apply** runs as a background job (Python supervisor; safe to leave the page) until finished or **Stop** (cooperative — finishes the current step). Refresh has the same Stop between publish stages. Missing visual **delivery** thumbnails need Refresh, not Repair.
