...
- Build actions under **System → Status**: **Site health** doctor flow — **Quick health check** / **Full health check** (read-only), **Review treatment** → **Apply treatment**, **Force full rebuild** (blocked on critical catalogue findings), plus **Activity** for verbose evidence. Legacy Refresh site files / Repair catalogue / Peek under the hood are folded into this page; Status no longer exposes them as primary CTAs.
- **System → Environment**: host environment report (PHP/Python/ffmpeg/launch path; best-effort disk/load/CPU/RAM; **host egress** latency/throughput distinct from the login visitor speed test; active Publish/Optimize/Repair/Site health locks) and install host-protection sanity check (managed `.htaccess` / `.user.ini` vs templates) with optional preview/repair (developer-only; legacy `?stab=security` redirects here)
- **Multi-operator safety**: legacy Publish / Optimize use `log/build.lock` / `log/optimize.lock`; Site health uses `log/site-health.lock`. Manual Repair Preview/Apply shares `log/catalog-repair.lock` with background catalogue prep (second operator gets busy). Site update refuses while any of those locks are active — it does not kill a live job.
