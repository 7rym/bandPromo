...
| **Linked visuals / SFX** | `media/visual/master/*`; `media/sfx/master/*`; **asset registry subset** | **Primary:** every audio/visual row whose catalogue home (`release_id`) is this campaign. **Also:** owned brand library/slots; masters referenced by owned playlists/galleries/pages (including foreign-home dependency copies so the PCF plays on a host without that other campaign). No upload originals or delivery in the package; SFX delivery rebuilt as `media/sfx/optimal/{ast_*}.mp3`. If a brand shell SFX only has optimal on the source, export promotes it to `media/sfx/master/` (and registers the row when missing) instead of dropping it. Track `display.cover` / `living_cover` refs resolve to visual asset ids so cover masters travel. Import remaps homes when stamp == source campaign; **empty/primary audio and visual homes are claimed to the package campaign** (same-id Demo import included). Foreign non-empty homes stay as packed. Import rebuilds the Files index from masters when originals are absent. Orphan homes used in containers are healed by Site health / post-import heal / Site update migration (not only by export). |
...
- Hide is campaign-level (`demo_campaign_id` / `demo_release_hidden` in `data/install-preferences.json`): demo campaign containers (playlists / pages / galleries) and Files media whose **catalogue home** is the demo campaign leave admin lists, Files pools, pickers, and the player. Demo Brand documents (including locked `bandpromo-default` when it is not Base) leave Branding/PBF when hidden. Brand assets / Sound effects stay only while still referenced by the **Base** brand or another non-demo brand. Hide is offered only after an operator-created campaign with a track is exposed on a playlist. If that operator catalogue is later deleted, the demo catalogue is shown again.
