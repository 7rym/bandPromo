...
- `original`: the exact user upload, preserved untouched (**write-once**; legal I/O after intake is download/delete/provenance only). Operators may **manually** discard the archival upload when a master exists (**System → Status → Storage**) to free host disk; Site health never does this automatically.
...
- **Discard archival upload (Status → Storage):** When a master exists on disk, operators may discard the write-once `original/` upload from **System → Status → Storage** (not Files) to reclaim space. The panel shows host free space, approximate install sizes, and how much eligible originals can free (audio / visual / sound effects). Masters and delivery stay; “Download original” becomes unavailable (honest 404). Locked demo media is excluded until unlocked on localhost. **Never** auto-discarded by Site health Treat/Force.
