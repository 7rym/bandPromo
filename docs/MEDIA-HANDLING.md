...
- after that point, operator-facing repair/editing tools work against the master representation, not against the intake upload
...
Files that exist only in **original/** (for example a failed upload that never registered a master) are not editable or playable until catalogue registration succeeds. Files that exist only as **masters** (Demo PCF / campaign import, no intake original) are listed in Files from the asset registry and are editable. Normal Files → Audio uploads create the master immediately and prepare delivery without waiting for Site health Force. Add Audio may optionally assign the new master to a campaign at upload time (or leave it **Orphan**); assignment stamps registry `release_id` and appends the campaign track pool. Playlist save republishes that playlist’s player payload so `/play` can load without a full site rebuild.

Bundled demo audio is **not** git-tracked. The entire `/media` tree is ignored. Demo assets arrive via setup import of `bandPromo-demo.pcf` (or local seed) and are built into the normal three-tier layout on the host. Admin Publish does not re-download demo packages. Sound effects use the same three-tier idea under `media/sfx/{original,master,optimal}` (login plays delivery MP3 when ready). Site health Treat/Force runs SFX delivery that materializes masters and builds tagless optimal MP3s when missing or stale (same helper as upload/import backfill); already-current deliveries are **kept**.
...
- Files → Visual lists the media files index (registry masters first). Publish and Site health Treat also scan `media/visual/master/` for unregistered `ast_*` masters and re-attach them to the registry and pool (never deletes those bytes). An empty or short `original/` folder alone does not mean masters are gone.
- Files → Audio follows the same master-first recover path: Publish / Site health Apply scan `media/audio/master/` for unregistered `ast_*` masters and re-register them into the registry and Files pool (never deletes those bytes; never invents `original/` from a master). Uncatalogued intake under `media/audio/original/` is registered on Apply/Publish only — Files → Audio does **not** list those originals as extra pool rows.
- **Do not** invent `original/` bytes from masters. Original is as-uploaded intake only and is disposable after the master exists; PCF/PBF are masters-only so hosts may legitimately lack originals. Pool, delivery, player, pickers, and admin downloads work from the **master**. There is no Files Original|Master list toggle. Operators download enriched masters (and side-car data that cannot live in master metadata) — not the archival upload. Product resolve (`visual_working_path`, covers, share, Files index) never falls back to original/legacy folders when the master is missing — fail loud. Never invent registry `original_filename` from the master id.
...
- **Metadata (locked):** keep camera **EXIF** as read-only provenance (`DateTimeOriginal` / GPS → registry `captured_at`). Write operator **title / description / keywords** as **IPTC Core via XMP** on the master only (not into EXIF editorial fields). Content autofix invents empty registry **title** (original filename stem, else Untitled) and **captured_at** (master mtime) without remuxing video masters in bulk — that path timed out shared-host Apply. Single-asset save still writes IPTC/XMP or Matroska tags. Description stays optional (never invented). Formats in scope: JPG/JPEG, PNG, WebP. Registry `master_width` / `master_height` are the master pixel size (Files list Dimensions); delivery variant sizes stay on `delivery.variants`.
...
- Rebuild is incremental (XXH3 skip-if-current / kept where available)
...
| `thumb` | Playlist row `.playlist-track-cover`, cover-flow, bio track list | 70–100px (delivery max edge **150px**) | Square-ish; shipped |
| `grid` | Admin Files pool/picker tiles; page gallery cells | ~112–188px CSS (+ retina); delivery max edge **320px** | Built for image roles after Treat; admin prefers grid → card → thumb |
| `card` / `optimal` | Player flip cover `.cover-art` inside `--card-size` (max 600px); admin fallback before grid exists | delivery max edge **720px** | Shipped |
...
- The Build tab now speaks in task-oriented operator language via **System → Status → Site health** (Quick/Full check, Review → Apply, Force). Legacy Publish / Optimize button labels in older hosts fold into that page.
- Audio + visual image delivery skip-if-current via master **XXH3** (`assets[].delivery.source_xxh3`). Operator Activity counts **built / kept / failed** (kept = fingerprint matched, existing deliverable left alone). Visual variants compare their expected contain dimensions against `delivery-contexts.json` (`max_edge` or `max_width` + `max_height`) so policy changes rebuild without a master change. Legacy audio `source_mtime` remains as a one-build migration fallback until XXH3 is recorded. Force with `BANDPROMO_FORCE_AUDIO_DELIVERY=1` / `BANDPROMO_FORCE_VISUAL_DELIVERY=1`. Requires Python `xxhash` (`pip install -r scripts/requirements.txt`).
...
| --- | --- | --- | --- |
| Upload audio source | `playlist-scan`, `audio-delivery`; cover extract for the uploaded masters | Run `playlist-scan` automatically in validation-only mode (validation report only — not the cover guarantee); run `audio-delivery` for uploaded files; run Files-owned `extract_upload_covers.py` for those masters (embedded art → Visual track-cover + `display.cover` + thumb/card inline). Do **not** mark sticky `image-delivery` / Site health Notifications when cover ingest succeeds. Campaign/orphan filter stays; playlist membership is unrelated. | Clean success when title/artist/cover tags + delivery succeed. Red C only when the file truly has no art. Site health Treat remains catch-up for leftovers. |
| Upload video source | `video-delivery` | Spawn async background delivery for all uploaded videos (MP4 copy, MOV/WEBM transcode, poster generation) | Toast when ready or failed; failed items keep Stop retrying in Notifications; hide from gallery pool until delivery is ready |
